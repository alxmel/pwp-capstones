"""
A.M.
Murder Mystery, final project.
"""

# Save the different examples as variables
...
def get_average_sentence_length(text):
    replacing_q = text.replace('?', '.')
    replacing_x = replacing_q.replace('!', '.')
    intro_split = replacing_x.split('.')
    length_total = 0
    list_elements = 0
    average_length = 0
    for element in intro_split:
        list_elements += 1
        length_total += len(element)
    average_length = length_total / list_elements
...
# Creating the Definition of our Model
class TextSample:
    ...
    def __repr__(self):
        return "{author}'s average sentence length is: {length}".format(author=self.author, length=self.average_sentence_length)
    

# Creating our TextSample Instances
murder_sample = TextSample(murder_note, 'murderer')
lily_sample = TextSample(lily_trebuchet_intro, 'Lily Trebuchet')
myrtle_sample = TextSample(myrtle_beach_intro, 'Myrtle Beach')
gregg_sample = TextSample(gregg_t_fishy_intro, 'Gregg T Fishy')


print("{name}'s similarity score to the murder letter is: {score}".format(name='Lily Trebuchet', score=find_text_similarity(murder_sample, lily_sample)))
print()
print("{name}'s similarity score to the murder letter is: {score}".format(name='Myrtle Beach', score=find_text_similarity(murder_sample, myrtle_sample)))
print()
print("{name}'s similarity score to the murder letter is: {score}".format(name='Gregg T Fishy', score=find_text_similarity(murder_sample, gregg_sample)))
print()

print('Lily Trebuchet is the killer!')
